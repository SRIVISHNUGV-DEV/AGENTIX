...
import { AgentIXConfig, SigningRequest } from './types.js';
import { RpcError, SignerRequiredError } from './errors.js';
...
export class ContractRegistry {
   ...
   * If no Signer was provided, the returned contract will still work for read calls
   * but writing will fail with SignerRequiredError when the SDK tries to submit.
   */
  send<T extends Contract>(name: ContractName, address?: string): T {
    const addr = address || (this.config as any)[name + 'Address'];
    if (!addr) throw new RpcError('No address configured for ' + name);
    if (!this.signer) {
      // Return a contract wired to a dummy runner that will throw on writes.
      // This avoids the "need signer upfront" problem for read-heavy flows.
      return new Contract(addr, ABIS[name], this.provider) as unknown as T;
    }
  ...
  getSignerAddress(): string {
    ...
    // Signer might not expose address synchronously, but ethers v6 Signer usually does
    return '0x'; // caller should use getSignerAddressAsync for reliability
  }
   ...
   * This is the core of AgentIX's commitment to transparent signing.
   */
  describe(
    name: ContractName,
    functionName: string,
