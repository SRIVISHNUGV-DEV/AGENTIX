    ...
    return (
        <div className="space-y-5">
            <div className="space-y-3">
                <button
                    onClick={() => {
                        const backendUrl = process.env.NEXT_PUBLIC_AGENT_CREDENTIALS_API_URL || 'http://127.0.0.1:3001'
                        window.location.href = `${backendUrl}/auth/oauth/google`
                    }}
                    className="flex w-full items-center justify-center gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3 text-sm font-medium transition hover:bg-zinc-800 hover:border-zinc-700"
                >
            ...
            <Button
                ...
                onClick={submit}
            >
                {mode === 'login' ? 'Sign in with email' : 'Create organization'}
            </Button>
