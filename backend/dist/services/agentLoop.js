...
exports.pauseAgentLoop = pauseAgentLoop;
exports.resumeAgentLoop = resumeAgentLoop;
exports.getAgentLoopState = getAgentLoopState;
exports.subscribeToLoopEvents = subscribeToLoopEvents;
exports.getActiveLoops = getActiveLoops;
exports.queueAgentTask = queueAgentTask;
 ...
 */
async function startAgentLoop(config, blockchainService) {
    ...
    const agent = await db.get("SELECT * FROM external_agents WHERE id = ?", [agentId]);
    if (!agent) {
        throw new Error(`Agent ${agentId} not found`);
    }
    // Check for valid credentials
    const credentials = await db.all(`SELECT * FROM credentials WHERE agent_id = ? AND expires_at > ?`, [agentId, Math.floor(Date.now() / 1000)]);
 ...
 */
async function stopAgentLoop(agentId) {
 ...
 */
function pauseAgentLoop(agentId) {
    const loop = activeLoops.get(agentId);
    if (!loop) {
        return null;
    }
 ...
 */
function resumeAgentLoop(agentId, blockchainService) {
    const loop = activeLoops.get(agentId);
 ...
 */
function getAgentLoopState(agentId) {
    const loop = activeLoops.get(agentId);
 ...
 */
async function executeLoopIteration(agentId, blockchainService) {
    const db = await (0, db_1.initDB)();
    const loop = activeLoops.get(agentId);
    ...
    try {
        ...
        const pendingTasks = await db.all(`SELECT * FROM agent_execution_queue
        ...
        await db.run(`UPDATE agent_execution_queue SET status = 'running', started_at = ? WHERE id = ?`, [Math.floor(Date.now() / 1000), task.id]);
 ...
 */
async function executeTask(agentId, task, blockchainService, timeoutMs) {
    const startTime = Date.now();
    ...
    try {
        ...
        if (blockchainActions.includes(task.action)) {
            ...
            const toolResult = await executeWithTimeout(agentTools.executeAction(task.action, params), timeout);
            return {
        ...
        switch (task.action) {
            case "sign_transaction":
                ...
                result = await executeWithTimeout(executeSignTransaction(agentId, params, blockchainService), timeout);
                break;
            case "api_call":
                result = await executeWithTimeout(executeApiCall(params), timeout);
                break;
            case "read_file":
            case "write_file":
 ...
 */
async function executeSignTransaction(agentId, params, blockchainService) {
    const { to, value, data } = params;
    const db = await (0, db_1.initDB)();
    // Get agent's wallet
    const wallet = await db.get(`SELECT w.* FROM wallets w
 ...
 */
function subscribeToLoopEvents(agentId, event, callback) {
    const loop = activeLoops.get(agentId);
    if (!loop) {
 ...
 */
async function queueAgentTask(agentId, action, params, scheduledAt) {
    const db = await (0, db_1.initDB)();
