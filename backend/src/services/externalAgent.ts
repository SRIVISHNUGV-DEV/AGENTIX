...
const DEFAULT_ENCRYPTION_KEY = crypto
    .createHash("sha256")
    .update("agentix-local-development-key")
    .digest("hex")

function getEncryptionKey(): Buffer {
    const configured = process.env.ENCRYPTION_KEY?.trim()
    const key = configured || DEFAULT_ENCRYPTION_KEY

    if(!configured && process.env.NODE_ENV === "production"){
        throw new AppError(500, "ENCRYPTION_KEY is required", false)
    }
